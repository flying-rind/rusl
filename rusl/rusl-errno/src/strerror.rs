...
pub(crate) fn errno_to_message(e: c_int) -> *const c_char {
...
// ===========================================================================
// strerror / strerror_l / __strerror_l — C ABI 导出的错误消息函数
// ===========================================================================
...
/// 内部实现: 将错误码 `e` 转换为对应的人可读错误描述字符串。
///
/// `loc` 指定 locale 句柄，用于消息翻译。Stage 0: `loc` 参数被忽略。
///
/// musl 中 `strerror_l` 是 `__strerror_l` 的弱别名。
///
/// 返回的指针指向静态只读数据，调用者不应修改或释放。
///
/// # C 签名
...
/// ```
#[no_mangle]
pub extern "C" fn __strerror_l(e: c_int, _loc: locale_t) -> *mut c_char {
    errno_to_message(e) as *mut c_char
}
...
/// 本函数等价于 `__strerror_l(e, loc)`（musl 中的弱别名）。
///
/// 返回的指针指向静态只读数据，调用者不应修改或释放。
///
/// # C 签名
...
/// ```
#[no_mangle]
pub extern "C" fn strerror_l(e: c_int, loc: locale_t) -> *mut c_char {
    __strerror_l(e, loc)
}
