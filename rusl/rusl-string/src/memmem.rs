...
#[no_mangle]
pub unsafe extern "C" fn memmem(h0: *const core::ffi::c_void, k: usize, n0: *const core::ffi::c_void, l: usize) -> *mut core::ffi::c_void {
    ...
    let mut i = 0usize;
    while i <= max_i {
        if unsafe { *haystack.add(i) } == first {
            // 检查剩余部分
            let mut found = true;
            for j in 1..l {
                if unsafe { *haystack.add(i + j) } != unsafe { *needle.add(j) } {
                    found = false;
                    break;
                }
            }
