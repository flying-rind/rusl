...
/// # 实现说明
///
/// 在 rusl 中，此函数通过 `rusl_internal::do_syscall!` 宏发起系统调用：
/// ```ignore
/// rusl_internal::do_syscall!(SYS_MREMAP, old, old_len, new_len, flags)
/// ```
...
pub(crate) unsafe fn sys_mremap(
    ...
    rusl_internal::do_syscall!(SYS_MREMAP, old, old_len, new_len, flags, 0usize) as *mut c_void
}
...
pub(crate) unsafe fn sys_mmap(
    ...
    rusl_internal::do_syscall!(SYS_MMAP, addr, len, prot, flags, fd, off) as *mut c_void
}
...
pub(crate) unsafe fn sys_munmap(addr: *mut c_void, len: usize) -> c_int {
    rusl_internal::do_syscall!(SYS_MUNMAP, addr, len) as c_int
}
