...
// 内部实现, isgraph_l 通过 weak_alias 符号别名指向此函数
fn __isgraph_l(c: c_int, l: locale_t) -> c_int;

// [Visibility]: External
// POSIX.1-2008 locale-aware 变体, weak_alias -> __isgraph_l
...
```rust
// 无符号减法区间检查, 编译为无分支代码
(c as u32).wrapping_sub(0x21) < 0x5e
// 通过 u32 区间检查: 0x21 到 0x7E（含）映射到 0 到 0x5D
// 时间复杂度 O(1), 无分支
```
...
Exported Interface:
  extern "C" fn isgraph(c: c_int) -> c_int;
    // [Visibility]: External
    // 判断 c 是否为可打印图形字符（0x21-0x7E, 排除空格）
    // 返回值: 图形字符返回非零值, 否则返回 0
  extern "C" fn isgraph_l(c: c_int, locale: locale_t) -> c_int;
    // [Visibility]: External
    // isgraph 的 locale-aware 变体
    // 在 C/POSIX locale 下与 isgraph 行为相同, locale 参数被忽略
    // 本模块保证对外提供该接口签名, ABI 兼容 C isgraph_l